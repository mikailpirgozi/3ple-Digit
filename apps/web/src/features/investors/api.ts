...
export const investorsApi = {
  ...
  getInvestors: (filters?: InvestorFilters): Promise<InvestorsResponse> =>
    api.get('/investors', filters),

  // Get investor by ID
  getInvestor: (id: string): Promise<Investor> => api.get(`/investors/${id}`),

  // Create new investor
  createInvestor: (data: CreateInvestorRequest): Promise<Investor> => api.post('/investors', data),
  ...
  // Delete investor
  deleteInvestor: (id: string): Promise<void> => api.delete(`/investors/${id}`),
  ...
  ): Promise<InvestorCashflow> => api.put(`/investors/${investorId}/cashflows/${cashflowId}`, data),
