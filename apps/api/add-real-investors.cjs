...
async function addRealInvestors() {
  console.log('🔄 Pridávam reálnych investorov bez mazania existujúcich dát...');
  ...
  try {
    ...
    // Pridať reálnych používateľov
    console.log('👥 Pridávam reálnych používateľov...');
    for (const user of realUsers) {
      try {
        ...
        console.log(`  ✅ Pridaný používateľ: ${user.name}`);
      } catch (error) {
        if (error.code === 'P2002') {
          console.log(`  ⚠️  Používateľ ${user.name} už existuje, preskakujem`);
        } else {
          console.error(`  ❌ Chyba pri pridávaní používateľa ${user.name}:`, error.message);
        }
      }
    }

    // Pridať reálnych investorov
    console.log('💼 Pridávam reálnych investorov...');
    for (const investor of realInvestors) {
      try {
        ...
        console.log(`  ✅ Pridaný investor: ${investor.name}`);
      } catch (error) {
        if (error.code === 'P2002') {
          console.log(`  ⚠️  Investor ${investor.name} už existuje, preskakujem`);
        } else {
          console.error(`  ❌ Chyba pri pridávaní investora ${investor.name}:`, error.message);
        }
      }
    }

    // Pridať reálne cashflows
    console.log('💰 Pridávam reálne cashflows...');
    for (const cashflow of realCashflows) {
      try {
        ...
        console.log(`  ✅ Pridaný cashflow: ${cashflow.amount.toLocaleString()} EUR pre ${realInvestors.find(i => i.id === cashflow.investorId)?.name}`);
      } catch (error) {
        if (error.code === 'P2002') {
          console.log(`  ⚠️  Cashflow už existuje, preskakujem`);
        } else {
          console.error(`  ❌ Chyba pri pridávaní cashflow:`, error.message);
        }
      }
    }

    console.log('✅ REÁLNE dáta úspešne pridané!');
    
    // Vypočítať celkový kapitál
    const totalCapital = realCashflows.reduce((sum, cf) => sum + cf.amount, 0);
    console.log(`💰 Celkový kapitál reálnych investorov: €${totalCapital.toLocaleString()}`);
    ...
    // Zobraziť všetkých investorov
    console.log('\\n💼 Všetci investori v databáze:');
    ...
    console.error('❌ Pridávanie zlyhalo:', error);
  } finally {
