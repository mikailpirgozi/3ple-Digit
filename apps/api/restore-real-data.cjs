...
async function restoreRealData() {
  console.log('🔄 Restoring REAL production data from SQL backup...');
  
  try {
    // First, clean current fake data
    ...
    // Real users data from your backup
    const realUsers = [
    ...
    // Real cashflows data (kapitál k 30.6.2025)
    const realCashflows = [
    ...
    // Create investors
    console.log('💼 Creating real investors...');
    ...
    // Create cashflows
    console.log('💰 Creating real cashflows...');
    for (const cashflow of realCashflows) {
      try {
        ...
        console.log(`  ✅ Created cashflow: ${cashflow.amount.toLocaleString()} EUR for ${realInvestors.find(i => i.id === cashflow.investorId)?.name}`);
      } catch (error) {
        if (error.code === 'P2002') {
          console.log(`  ⚠️  Cashflow already exists, skipping`);
        } else {
          console.error(`  ❌ Error creating cashflow:`, error.message);
        }
      }
    }

    console.log('✅ REAL data restore completed!');
    
    // Calculate totals
    const totalCapital = realCashflows.reduce((sum, cf) => sum + cf.amount, 0);
    console.log(`💰 Total investor capital: €${totalCapital.toLocaleString()}`);
